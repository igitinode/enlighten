# Vitepress 提供内置 markdown 扩展 {#vp-build-in-md}

展示一些 VP 提供的 md 扩展展示效果，方便开发查对。
...
标题会自动应用锚点。可以在主题配置中使用 `markdown.anchor` 选项配置锚点的渲染。
...
默认情况下，生成的页面和内部链接带有 `.html` 后缀。使用配置属性 `cleanUrls: true` 可以去掉 URL`.html`后缀。

### 外部链接 {#external-links}

外部链接渲染出来带有 `target="_blank" rel="noreferrer"`：
...
可以在主题配置文档使用 `markdown.toc` 选项配置 TOC 的呈现效果。
...
此外，可以通过在`站点配置`中添加以下内容来全局设置自定义标题，如果不是用英语书写，这会很有帮助：
